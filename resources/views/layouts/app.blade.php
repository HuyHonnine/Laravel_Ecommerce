                        ...
                        <ul class="nav navbar-nav d-flex flex-row">
                            <li class="dropdown messages-menu text-white">
                                ...
                                <ul class="dropdown-menu">
                                    @php
                                        $list = app('App\Http\Controllers\HomeController')->message_list();
                                        $newMessageCount = $list->where('status', 0)->count();
                                    @endphp
                                    <li class="header">Bạn có {{ $newMessageCount }} đánh giá mới từ khách!</li>
                                    @foreach ($list as $key => $listMessage)
                                        <li>
                                            <ul class="menu">
                                                <li>
                                                    @if ($listMessage->status == '0')
                                                        <a href="{{ route('orderDetail', $listMessage->order->id) }}"
                                                            class="">
                                                            <div class="pull-left">
                                                                <img src="{{ asset('uploads/user/' . $listMessage->user->image) }}"
                                                                    class="img-circle" alt="User Image">
                                                            </div>
                                                            <h4>
                                                                {{ $listMessage->user->name }}
                                                                <small><i class="fa fa-clock-o"></i>
                                                                    {{ $listMessage->date_review->diffForHumans() }}</small>
                                                            </h4>
                                                            <div class="flex items-center justify-between">
                                                                <p>{{ $listMessage->content }}</p>
                                                                <span
                                                    ...
                                                    @else
                                                        <a href="#" class="">
                                                            ...
                                                            <p>{{ substr($listMessage->content, 20) }} ...</p>
                                                        </a>
                                                    @endif
                                                </li>
                                                <!-- end message -->
                            ...
                            @role('admin')
                                @php
                                    $list = app('App\Http\Controllers\HomeController')->order_list();
                                    $newOrdersCount = $list->where('status', 0)->count();
                                @endphp
                                <li class="dropdown notifications-menu text-white">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li class="header font-bold">Bạn có {{ $newOrdersCount }} đơn hàng mới
                                            !</li>
                                        @foreach ($list as $key => $listOrder)
                                            <li>
                                                <ul class="menu">
                                                    <li class="">
                                                        @if ($listOrder->status == '0')
                                                            <a href="{{ route('orderDetail', $listOrder->id) }}"
                                                                class="group bg-green-500 text-white hover:bg-gray-400">
                                                                <p
                                                                    class="group-hover:font-semibold group-hover:italic group-hover:text-black">
                                                                    <i class="ri-shopping-basket-line text-xl"></i>
                                                                    Đơn hàng mới từ {{ $listOrder->user->name }}
                                                                </p>
                                                            </a>
                                                        @else
                                                            <a href="">
                                        ...
                                        <li class="footer" class="hover:text-xl hover:font-bold">
                                            <a href="{{ route('list-order') }}">xem danh sách đơn</a>
                                        </li>
                                    </ul>
                                </li>
                            @endrole
                    ...
                    <ul class="sidebar-menu" data-widget="tree">
                        ...
                        @role('admin')
                            ...
                            <li class="treeview {{ $segment == 'order' ? 'active' : '' }}">
                                <a href="#">
                                    <i class="ri-store-2-line"></i><span> Quản lý đơn hàng</span>
                                    <span class="pull-right-container">
                                        <i class="fa fa-angle-left pull-right"></i>
                                    </span>
                                </a>
                                <ul class="treeview-menu">
                                    <li><a href="{{ url('/list-order') }}"><i class="fa fa-angle-right"></i>Danh
                                            sách đơn hàng</a></li>
                                    <li><a href="{{ url('/list-order') }}"><i class="fa fa-angle-right"></i>Danh
                                            đánh giá đơn hàng</a></li>
                                </ul>
                            </li>
